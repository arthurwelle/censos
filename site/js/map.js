...
function medirLegenda(ctx, k) {
  ...
  return {
    ...
    w: pad * 2 + sw + gap + larguraTxt + gap + barra + gap + pctW,
...
function desenharTexto(ctx, k, larg, alt, titulo, creditos) {
  ...
  const centrado = (txt, y, cor, tam, peso) => {
    ctx.font = `${peso} ${tam * k}px ${PILHA_FONTE}`;
    const x = (larg - ctx.measureText(txt).width) / 2;
  ...
  if (titulo) centrado(titulo, 18 * k, themeVar('--text', '#e8ecf1'), 16, '600');
  if (creditos) centrado(creditos, alt - 24 * k, themeVar('--text-2', '#aab4bf'), 10.5, '400');
}
...
export async function montarCanvas({ titulo = '', creditos = '',
  ...
  if (comLegenda) desenharLegenda(ctx, k, out.width, out.height, cantoLegenda);
...
export function comporGrade(paineis, { titulo = '', creditos = '',
  ...
  const mleg = comLegenda ? medirLegenda(medida, k) : null;

  const gradeW = cols * pw + (cols - 1) * vao;
  const alturaTitulo = titulo ? 34 * k : 0;
  const alturaLegenda = mleg ? mleg.h + 14 * k : 0;
  const alturaFonte = creditos ? 24 * k : 0;

  const out = document.createElement('canvas');
  out.width = Math.round(Math.max(gradeW, mleg?.w ?? 0) + mar * 2);
  ...
  paineis.forEach((p, i) => {
    ...
    ctx.font = `600 ${13 * k}px ${PILHA_FONTE}`;
    ctx.strokeStyle = themeVar('--bg-page', '#0f1216');
    ctx.lineWidth = 3.5 * k;
    ctx.strokeText(String(p.ano), x + 9 * k, y + 7 * k);
    ctx.fillStyle = themeVar('--text', '#e8ecf1');
    ctx.fillText(String(p.ano), x + 9 * k, y + 7 * k);
  });

  if (mleg) {
    // As CORES valem para a grade inteira: o domínio dos cortes é o mesmo em
    // todos os painéis, e é por isso que a figura se compara. A barra de
    // população, não — ela é a distribuição do último censo desenhado. Dizer
    // qual é mais honesto do que apagar a barra, que é informação boa.
    const ultimoAno = [...paineis].reverse().find((p) => p.canvas)?.ano;
    pintarLegenda(ctx, k, mleg, (out.width - mleg.w) / 2,
                  y0 + rows * ph + (rows - 1) * vao + 14 * k,
                  `cores iguais nos ${paineis.length} painéis · ` +
                  `barra: % da população em ${ultimoAno}`);
  }

  const centrado = (txt, y, cor, tam, peso) => {
    ctx.font = `${peso} ${tam * k}px ${PILHA_FONTE}`;
    ctx.fillStyle = cor;
    ctx.fillText(txt, (out.width - ctx.measureText(txt).width) / 2, y);
  };
  if (titulo) centrado(titulo, mar, themeVar('--text', '#e8ecf1'), 16, '600');
  if (creditos) {
    centrado(creditos, out.height - mar - 12 * k,
             themeVar('--text-2', '#aab4bf'), 10.5, '400');
  }
  return out;
}
